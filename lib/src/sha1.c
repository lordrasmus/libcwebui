/*
SHA-1 in C
Based on Code By Steve Reid <steve@edmweb.com>
100% Public Domain
...
struct sha_context* WebserverSHA1Init(void){
	...
	SHA1_CTX *ctx = WebserverMalloc( sizeof( SHA1_CTX ) );
	memset( ctx, 0 , sizeof( SHA1_CTX ) );
	
	SHA1Init(ctx);
	
	return (struct sha_context*)ctx;
}

int                 WebserverSHA1Update(struct sha_context* sha_ctx,const void* data,size_t len){
	
	SHA1Update( (SHA1_CTX*) sha_ctx, (unsigned char*)data, len);
	
	return 0;
}
int                 WebserverSHA1Final(struct sha_context* sha_ctx,unsigned char* data){
	
	SHA1Final(data, (SHA1_CTX*) sha_ctx);
