...
#ifdef WEBSERVER_USE_MBEDTLS3_CRYPTO
...
#include <dmalloc/dmalloc.h>
	...
	#error "mbedtls3.c requires mbed TLS 3.0.0 or later - use mbedtls2.c for older versions"
#endif
...
int initOpenSSL(void) {
	...
	if (psa_status != PSA_SUCCESS) {
		LOG(SSL_LOG, ERROR_LEVEL, 0, "psa_crypto_init() failed: %d", (int)psa_status);
		return -1;
	}
...
int VISIBLE_ATTR WebserverSSLTestKeyfile(char *keyfile) {
	...
	if (!error) {
		/* Need RNG for key parsing in mbedTLS 3.x */
		mbedtls_entropy_context test_entropy;
		mbedtls_ctr_drbg_context test_rng;

		mbedtls_entropy_init(&test_entropy);
		mbedtls_ctr_drbg_init(&test_rng);
		mbedtls_ctr_drbg_seed(&test_rng, mbedtls_entropy_func, &test_entropy, NULL, 0);

		ret = mbedtls_pk_parse_keyfile(&test_key, keyfile, NULL,
		                               mbedtls_ctr_drbg_random, &test_rng);
		if (ret != 0) {
			LOG(SSL_LOG, ERROR_LEVEL, 0, "Can't read key: file %s", keyfile);
			error = 1;
		}

		mbedtls_ctr_drbg_free(&test_rng);
		mbedtls_entropy_free(&test_entropy);
	}
...
#endif /* WEBSERVER_USE_MBEDTLS3_CRYPTO */
