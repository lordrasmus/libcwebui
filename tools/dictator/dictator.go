...
func scan_dir ( path string , subpath string) []string {
	...
	for num, f := range files {
		...
		if err != nil {
			...
			if fi.IsDir() {
				//continue
				//fmt.Println("Source is a directory")
				var ret_sub = scan_dir( path + "/" + subpath + "/" + f.Name() , "/" + subpath + f.Name() )
				for _, f := range ret_sub {
					//fmt.Println("l : " + f )
					ret = append(ret ,  f )
				}
				//fmt.Println(ret_sub)
				...
				//fmt.Println("name : " + subpath + "/" + f.Name() )
				
				ret = append(ret ,  subpath + "/" + f.Name() )
			}
		}
	}
...
func main() {
	...
	for num, f := range files {
		...
		file, err := os.Open(path + "/" + f ) // For read access.
		if err != nil {
			fmt.Println("cont")
			continue
		}
